...
def calculate_count_srk(inp: FoxProInput, slvst: ArticleRecord, lang: str = "ru") -> List[List[Any]]:
    ...
    # Определяем признак посягательства на жизнь/здоровье для ст. 55
    _is_lh = _is_life_health(inp.article_code)

    # ── Ст. 55 ч.8: блокировка всей ст.55 ──
    # Абзац 1: убийство (ст.99) и половые преступления против н/летних
    _art55_blocked = _check_art55_part8(inp.article_code, ln_fs1r14p1)

    if _art55_blocked:
        # Коэффициенты ст.55 НЕ применяются; отмечаем наличие ПС/УДР если есть
        if inp.fs1r041p1 == "1" or inp.fs1r042p1 == "1":
            a_nakaz[5][11] = a_nakaz[5][11] + 1
    else:
        # ---- Ст. 55 ч.3: процессуальное соглашение / УДР ----
        if inp.fs1r041p1 == "1" or inp.fs1r042p1 == "1":
            # По умолчанию: не более 1/2 максимального срока
            ln_mnoj_udp = 1
            ln_del_udp = 2

            if slvst.hard == "3" and _is_lh:
                # Тяжкое с посягательством на жизнь/здоровье → 3/4
                ln_mnoj_udp = 3
                ln_del_udp = 4
            elif slvst.hard == "4" and not _is_lh:
                # Особо тяжкое без посягательства на жизнь/здоровье → 3/4
                ln_mnoj_udp = 3
                ln_del_udp = 4
            elif slvst.hard == "4" and _is_lh:
                # Особо тяжкое с посягательством на жизнь/здоровье → 4/5
                ln_mnoj_udp = 4
                ln_del_udp = 5

            a_nakaz[5][11] = a_nakaz[5][11] + 1
        else:
            # ---- Ст. 55 ч.2: смягчающие обстоятельства без отягчающих ----
            if _has_value(inp.mitigating) and not _has_value(inp.aggravating):
                if slvst.hard in ("1", "2") or slvst.prest == "2":
                    # п.1: проступок, небольшой или средней тяжести → 1/2
                    ln_mnoj, ln_del = 1, 2
                elif slvst.hard == "3" and not _is_lh:
                    # п.2: тяжкое преступление (без жизнь/здоровье) → 2/3
                    ln_mnoj, ln_del = 2, 3
                elif (slvst.hard == "3" and _is_lh) or (slvst.hard == "4" and not _is_lh):
                    # п.3: тяжкое с жизнь/здоровье ИЛИ особо тяжкое → 3/4
                    ln_mnoj, ln_del = 3, 4
                elif slvst.hard == "4" and _is_lh:
                    # п.4: особо тяжкое с жизнь/здоровье → 4/5
                    ln_mnoj, ln_del = 4, 5
...
# ═══════════════════════════════════════════════════════════════════
# Ст. 55: вспомогательные функции
# ═══════════════════════════════════════════════════════════════════

# ── Ст.55 ч.2-3: преступления против жизни и здоровья (ст. 99-119) ──
_LIFE_HEALTH_PREFIXES = frozenset(str(n).zfill(3) for n in range(99, 120))


def _is_life_health(article_code: str) -> bool:
    """Посягательство на жизнь или здоровье (глава 1 Особ. части: ст.99-119)."""
    return len(article_code) >= 3 and article_code[:3] in _LIFE_HEALTH_PREFIXES


# ── Ст.55 ч.8: блокировка ст.55 ──────────────────────────────────
#
# Абзац 1: ВСЯ ст.55 не применяется при:
#   1) умышленном причинении смерти (ст.99)
#   2) преступлениях против половой неприкосновенности н/летних
#      (ст.120 ч.3-1/3-2/4, ст.121 ч.3-1/3-2/4, ст.121-1,
#       ст.122 все ч., ст.123 ч.2, ст.124 все ч.,
#       ст.134 все ч., ст.144 все ч., ст.312 ч.2-3)
#   Исключение: н/летний преступник + потерпевший 14-18 лет
#
# Абзац 2 (TODO): ч.2 и ч.3 не применяются при насилии против
#   н/летних — требует входной флаг, пока не реализовано.

# Статьи ЦЕЛИКОМ о н/летних (все части)
_SEXUAL_MINOR_FULL = frozenset({"122", "124", "134", "144"})

# Подстатьи целиком о н/летних
_SEXUAL_MINOR_SUB = frozenset({"12101"})  # ст.121-1

# Конкретные коды article_code (определённые части)
_SEXUAL_MINOR_EXACT = frozenset({
    "1200031", "1200032", "1200004",  # ст.120 ч.3-1, 3-2, 4
    "1210031", "1210032", "1210004",  # ст.121 ч.3-1, 3-2, 4
    "1230002",                         # ст.123 ч.2
    "3120002", "3120003",              # ст.312 ч.2, 3
})

# Коды с малолетним потерпевшим (< 14): исключение ч.8 НЕ действует
_MALOLETNIY_CODES = frozenset({"1200004", "1210004"})  # ч.4 ст.120, 121


def _is_sexual_crime_against_minor(article_code: str) -> bool:
    """Преступление против половой неприкосновенности н/летних (п.42 ст.3 УК)."""
    if len(article_code) < 3:
        return False
    if article_code[:3] in _SEXUAL_MINOR_FULL:
        return True
    if len(article_code) >= 5 and article_code[:5] in _SEXUAL_MINOR_SUB:
        return True
    return article_code in _SEXUAL_MINOR_EXACT


def _check_art55_part8(article_code: str, offender_age: int) -> bool:
    """Ст.55 ч.8 абз.1: вся ст.55 заблокирована?

    Returns True если коэффициенты ст.55 НЕ должны применяться.
    """
    if not article_code or len(article_code) < 3:
        return False

    # 1. Умышленное причинение смерти (ст.99) — без исключений
    if article_code[:3] == "099":
        return True

    # 2. Половая неприкосновенность н/летних
    if _is_sexual_crime_against_minor(article_code):
        # Исключение: н/летний преступник + потерпевший 14-18
        if 0 < offender_age < 18:
            # ч.4 (малолетний < 14) → исключение НЕ работает
            if article_code in _MALOLETNIY_CODES:
                return True
            # Остальные (потерпевший мог быть 14-18) → in dubio pro reo
            return False
        return True
